...
<Steps>
  ...
  <Step title="Developer">
    ...
    <Card title="Input" icon="file-import">
      `{prd}` + `{architecture}` + Controllers Instructions
    </Card>
    <Card title="Steps (Chained Prompts)" icon="list-check">
...
Two modes: **manual** and **auto**. You can toggle between them from the TUI.

What happens in each mode depends on the agent's interactivity, chained prompts, and controller setup. [Learn more →](#)

**Four behaviors emerge:**
...
| **Forced** | Designed to run automatically but auto mode is off. Falls back to user drives with a warning. |