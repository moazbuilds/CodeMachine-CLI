...
export async function getActiveTemplate(
	...
	try {
		...
		console.warn(
			`Failed to read active template from tracking file: ${error instanceof Error ? error.message : String(error)}`,
		);
		return null;
	}
}
 ...
 */
export async function hasTemplateChanged(
	cmRoot: string,
	templateName: string,
): Promise<boolean> {
 ...
 * Supports both absolute paths and relative paths (relative to CodeMachine templates dir).
 */
export async function getTemplatePathFromTracking(
	cmRoot: string,
): Promise<string> {
	const activeTemplate = await getActiveTemplate(cmRoot);
	...
	if (path.isAbsolute(activeTemplate)) {
		return activeTemplate;
	}

	// Check if template exists in project's .codemachine directory first
	const projectTemplatePath = path.join(cmRoot, activeTemplate);
