	...
	if (!Array.isArray(obj.steps)) {
		...
		obj.steps.forEach((step, index) => {
			...
			if (candidate.type === "module") {
				...
				// Allow either promptPath (file reference) or prompt (inline string)
				const hasPromptPath = typeof candidate.promptPath === "string";
				const hasInlinePrompt =
					typeof (candidate as { prompt?: unknown }).prompt === "string";
				if (!hasPromptPath && !hasInlinePrompt) {
					errors.push(
						`Step[${index}] must have either promptPath or prompt as a string`,
					);
				}
				...
				if (
					candidate.executeOnce !== undefined &&
					typeof candidate.executeOnce !== "boolean"
				) {
				...
				if (candidate.module !== undefined) {
					if (!candidate.module || typeof candidate.module !== "object") {
						...
						if (moduleMeta.behavior !== undefined) {
							if (
								!moduleMeta.behavior ||
								typeof moduleMeta.behavior !== "object"
							) {
								...
								if (
									behavior.type !== "loop" ||
									behavior.action !== "stepBack"
								) {
									errors.push(
										`Step[${index}].module.behavior must be { type: 'loop', action: 'stepBack', ... }`,
									);
								}
								if (typeof behavior.steps !== "number" || behavior.steps <= 0) {
									errors.push(
										`Step[${index}].module.behavior.steps must be a positive number`,
									);
								}
								if (
									behavior.trigger !== undefined &&
									typeof behavior.trigger !== "string"
								) {
									errors.push(
										`Step[${index}].module.behavior.trigger must be a string if provided`,
									);
								}
								if (
									behavior.maxIterations !== undefined &&
									typeof behavior.maxIterations !== "number"
								) {
									errors.push(
										`Step[${index}].module.behavior.maxIterations must be a number`,
									);
								}
							}
						}
