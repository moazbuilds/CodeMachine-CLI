  ...
  mcp: [
    {
      ...
      only: ['run_agents', 'get_agent_status'],  // Limit available tools
      targets: ['code-generator', 'test-runner', 'doc-writer'],  // Allowed sub-agents
    },
  ],
}
```
...
<Card title="See Sub-Agent Example" icon="book-open" href="/build-workflows/workflow-examples#sub-agents-blueprint-orchestration">
...
<Warning>
  Both controller and all step agents must have `workflow-signals` MCP configured for autonomous mode to work. Without it, the controller can't approve transitions.
</Warning>
